'if solvesub_ngramcaching=0 then
	select case map2b(0,curr_symbol,lp,k)
		case 0
			z1=h3(sol(j),sol(j+1),sol(j+2))
			if z1>0 then
				for i=0 to abc_sizem1
					blt=bh6(z1,h3(sol(j+3),sol(j+4),i))
		...
		case 1
			...
			if z1>0 then
				for i=0 to abc_sizem1
					blt=bh6(z1,h3(sol(j+3),i,sol(j+5)))
					if blt>bls then bls=blt:new_letter=i
				next i
			end if
		case 2
		...
		case 5
			...
			if z2>0 then
				for i=0 to abc_sizem1
					blt=bh6(h3(i,sol(j+1),sol(j+2)),z2)
					if blt>bls then bls=blt:new_letter=i
				next i
			end if
	end select
