		...
		'if ni=1 then 'input + first hidden state
			'for nb=0 to nn_hid1w-1
				...
				for nj=0 to nn_rec-1
					'nn_state=nn_state and table1(abc1(sol(j+nj))+(nj*nn_abc))
					nn_state=nn_state and table1(bc_key(k,j+nj)+(nj*nn_abc)-65) 'ngram_alphabet_size
...
'	else 'last layer
		...
		'i64=__builtin_popcountll(nn_state and table2(abc1(sol(j+nn_rec))))
		i64=__builtin_popcountll(nn_state and table2(bc_key(k,j+nn_rec)-65))
'	end if
'next ni
