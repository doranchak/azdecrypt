...
if d>mc then
	...
	rng(k,1+map2(curr_symbol,lp,0),state)
	j=map2(curr_symbol,lp,k)
	select case ngram_system
		...
		case 2 'bh
			select case ngram_size
				...
				case 8
					new_letter=abc_size
					#include "solver_pickletter_bh8_bg2.bi"
				case 10
					new_letter=abc_size
					#include "solver_pickletter_bh10_bg2_b.bi"
				case else 'random
					rng(k,1+map2(curr_symbol,lp,0),state)
					j=map2(curr_symbol,lp,k)
			end select
		case 3 'nn
			#include "solver_randomnewletter.bi"
			if map2b(curr_symbol,lp,k)=0 then
				#include "solver_ngram_nn1_4_lp.bi" 'bugged?
				new_letter=argmax
			else
				new_letter=abc_size
			end if
